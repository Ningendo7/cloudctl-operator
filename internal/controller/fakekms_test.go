...
// fakeKey and fakeKMSClient are a minimal in-memory stand-in for the real
// KMS client, implementing cloudctlaws.KMSClient, so the controller's own
// wiring (KMS key reconciliation as part of a full Reconcile()) can be
// tested without hitting real AWS. Detailed KMS behavior (alias
// resolution, adoption, deletion scheduling) is already covered by
// internal/resources/kms's own unit tests; this fake only needs to be
